...
enrollment_by_grade_dict = []


for code in school_codes:
    
    response = requests.get(f'https://schoolreportcardstorage.table.core.windows.net/EnrollmentByGradeNEW()?$filter=PartitionKey%20eq%20%27{code}%27{url_appendix}',
                                headers={
    ...
    #x = x.reset_index()
    enrollment_by_grade_dict.append(x)



enrollment_by_grade = pd.concat(enrollment_by_grade_dict)
enrollment_by_grade.to_csv('enrollment_by_grade.csv', index=False)
